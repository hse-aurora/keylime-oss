...
class Route:
    ...
    def validate_abs_path(path):
        ...
        """
        if Route.PATH_ABSOLUTE_REGEX.match(path):
            return True
        else:
            return False
    ...
    def _parse_pattern(self, pattern):
        """Parses the given pattern into (1) parts which are expected to appear verbatim in matching paths and (2)
        ...
        :param pattern: The pattern to parse and store in the route

        :raises: :class:`InvalidPathOrPattern`: The given pattern is invalid
        """
        pattern_segments = Route._split_path(pattern)
        ...
        self._pattern_segments = pattern_segments

    def capture_params(self, path):
        ...
        for i in range(len(pattern_segments)):
            ...
            if isinstance(pattern_segments[i], str):
                if pattern_segments[i] == path_segments[i]:
                    continue
                else:
                    raise PatternMismatch(
                        f"segment '{path_segments[i]}' of path '{path}' does not match '{pattern_segments[i]}' "
                        f"from route pattern '{self.pattern}'"
                    )

            prefix = pattern_segments[i]["prefix"]
            param_name = pattern_segments[i]["param"]
    ...
    def call_action(self, controller_inst, path, params={}):
        ...
        try:
            ...
            if err.__traceback__ and err.__traceback__.tb_next == None:
                raise ActionDispatchError(str(err))
            else:
                raise err
